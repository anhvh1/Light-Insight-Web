...
export function MapLayoutManagerPanel({ cameras }: { cameras: CameraResponse[] }) {
  ...
  const geoStyleUrl = useMemo(() => {
    ...
    return `${getApiBaseUrl()}${rawUrl.startsWith('/') ? '' : '/'}${rawUrl}`;
  ...
  const positionsByCamera = useMemo(() => {
    ...
    return new Map(positionList.map((position) => [position.cameraId, position]));
  }, [positions]);
  ...
  const cameraById = useMemo(() => {
    const cameraList = Array.isArray(cameras) ? cameras : [];
    return new Map(cameraList.map((camera) => [camera.cameraId, camera]));
  }, [cameras]);
  ...
  const createCameraMarkerElement = () => {
    ...
    label.style.background = 'var(--t0)';
    label.style.color = 'var(--bg0)';
    ...
    rotateHandle.style.background = 'var(--accent)';
    rotateHandle.style.border = '2px solid var(--bg0)';
    ...
    const createScaleHandle = (role: string) => {
      ...
      handle.style.border = '2px solid var(--bg0)';
    ...
    fovLeft.style.background = 'var(--orange)';
    fovLeft.style.border = '2px solid var(--bg0)';
    ...
    fovRight.style.background = 'var(--orange)';
    fovRight.style.border = '2px solid var(--bg0)';
  ...
  const updateCameraMarkerElement = (
    ...
    if (iconWrap) {
      ...
      iconWrap.style.border = params.selected ? '1px dashed rgba(0, 194, 255, 0.7)' : 'none';
      ...
      return (
        <Box key={node.map.id}>
          ...
          {node.children.length > 0 && (
            <Stack gap="xs" mt="xs">
              {renderMapTree(node.children, depth + 1)}
            </Stack>
          )}
        </Box>
      );
  ...
  return (
    <Stack gap="lg" style={{ height: '100%', minHeight: 0, backgroundColor: 'var(--bg0)' }}>
      <Group justify="space-between" align="center" px="md" pt="md">
        <Stack gap={0}>
          <Text size="xl" fw={700} style={{ color: 'var(--t0)', letterSpacing: '-0.02em' }}>
            {t('pages.maps.title')}
          </Text>
          <Text size="xs" style={{ color: 'var(--t1)' }}>
        ...
        <Group gap="sm">
          {positionsDirty && (
            <Badge 
              variant="outline" 
              color="orange" 
              styles={{ root: { borderColor: 'var(--orange)', color: 'var(--orange)', background: 'rgba(255, 140, 0, 0.1)' } }}
            >
          ...
          <Button
            ...
            style={{ backgroundColor: 'var(--accent)', color: 'var(--bg0)' }}
          >
      ...
      >
              ...
              <Stack gap="md" style={{ flex: 1, minHeight: 0 }}>
                <Group justify="space-between" align="center">
                  ...
                  <Badge variant="light" color="blue" size="xs" style={{ backgroundColor: 'rgba(0, 194, 255, 0.1)', color: 'var(--accent)' }}>
                    {filteredCameras.length} {t('pages.maps.sidebar.devicesCount', { defaultValue: 'thiết bị' })}
                  </Badge>
                </Group>
                <TextInput
        ...
        >
          ...
          >
            <Group justify="space-between" align="center">
              <Stack gap={0}>
                <Text size="xs" fw={500} style={{ color: 'var(--t2)', textTransform: 'uppercase', letterSpacing: '0.05em' }}>
                  {activeMap?.type === 'Geo'
                    ? t('pages.maps.panel.geoHint')
                    : t('pages.maps.panel.imageHint')}
                </Text>
                <Group gap="sm">
                  <Text size="lg" fw={700} style={{ color: 'var(--t0)' }}>
              ...
              <Group gap="sm">
                {activeMap?.type === 'Image' && (
                  <FileInput
                    ...
                    leftSection={<IconUpload size={14} style={{ color: 'var(--accent)' }} />}
                    size="xs"
                    styles={{
                      input: { 
                        backgroundColor: 'var(--bg3)', 
                    ...
                    onChange={(file: File | null) => file && uploadImageMutation.mutate(file)}
                  />
                )}
                <Button
                  variant="filled"
                  ...
                  style={{ 
                    backgroundColor: canSave ? 'var(--accent)' : 'var(--bg3)', 
                    color: canSave ? 'var(--bg0)' : 'var(--t2)',
                    fontWeight: 700
                  }}
                >
      ...
      >
        <Stack gap="md">
          <TextInput
            ...
            styles={{ input: { backgroundColor: 'var(--bg3)', borderColor: 'var(--border-dim)', color: 'var(--t0)' } }}
          />
          <Select
