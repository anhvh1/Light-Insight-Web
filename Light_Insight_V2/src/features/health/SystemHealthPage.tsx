  ...
  useEffect(() => {
    ...
    connection.on('ReceiveAuditLog', (newLog: AuditLog) => {
      queryClient.setQueryData(['audit-logs-infinite'], (old: any) => {
        ...
        return { ...old, pages: [ { ...firstPage, Data: [newLog, ...(firstPage.Data || [])], TotalRow: (firstPage.TotalRow || 0) + 1 }, ...old.pages.slice(1) ] };
      });
    });
      ...
      queryClient.setQueryData(['system-health-status'], (old: any) => {
        ...
        const newConnectors = old.Data.Connectors.map((c: any) => {
          if (c.Name === state.Name) {
            return { ...c, Latency: `${state.LatencyMs}ms`, Stats: `${state.OnlineCameras} / ${state.TotalCameras}`, Status: state.Status };
          }
          return c;
        });
    ...
    connection.start().catch(err => console.error('Audit Hub Failed: ', err));
    return () => { connection.stop(); };
  }, [queryClient]);

  const handleRefresh = () => { refetchHealth(); refetchLogs(); };
    ...
    <div className="flex flex-col h-full overflow-hidden font-sans">
      ...
      <main className="flex-1 flex overflow-hidden gap-px">
        <div className="w-1/2 flex flex-col overflow-hidden bg-bg-0">
          <div className="shrink-0 flex flex-col min-h-0 overflow-hidden border-b border-white/5 pb-2">
            ...
            <div className="overflow-x-auto px-3 scrollbar-none">
              <div className="flex gap-2">
                {connectors.map((c, i) => {
                  ...
                  return (
                    <div key={i} onClick={() => setSelectedConnectorId(connectorIp)} className={cn("bg-bg-2 border rounded-lg p-2.5 flex flex-col gap-1 shadow-sm transition-all cursor-pointer min-w-[180px]", isSelected ? "border-psim-green shadow-psim-green/10" : "border-white/5 hover:border-white/10", c.Status === 'OFFLINE' && "opacity-60 grayscale-[0.5]")}>
                      ...
                      <div className="flex justify-between text-[10px]"><span className="text-t-2">Độ trễ</span><span className={cn("font-mono font-bold", getStatusColor(c.Status))}>{c.Latency}</span></div>
          ...
          <div className="flex-1 flex flex-col min-h-0 overflow-hidden">
            <div className="flex-1 flex flex-col min-h-0 overflow-hidden border-b border-white/5">
              <div className="sticky top-0 bg-bg-0 z-10 px-3 py-2 shrink-0 border-b border-white/5"><h2 className="text-[10px] font-mono font-bold text-t-2 uppercase tracking-[0.12em]">Hạ tầng & Hệ thống lưu trữ</h2></div>
            ...
            <div className="flex-1 flex flex-col min-h-0 overflow-hidden">
              <div className="sticky top-0 bg-bg-0 z-10 px-3 py-2 shrink-0 border-b border-white/5"><h2 className="text-[10px] font-mono font-bold text-t-2 uppercase tracking-[0.12em]">DANH SÁCH THIẾT BỊ</h2></div>
                      ...
                      <div className="grid grid-cols-2 gap-2">
                        {devices.map((device, i) => (
                          <div key={i} className="flex items-center justify-between gap-2 px-2 py-1.5 bg-bg-2/30 border border-white/5 rounded-lg hover:bg-bg2 transition-all group">
                            <div className="flex items-center gap-2 min-w-0"><span className="text-[11px]">{device.Type === 'hardware' ? '🛡️' : '📷'}</span><span className={cn("text-[10px] font-medium truncate uppercase", device.Type === 'hardware' ? "text-t0 font-bold" : "text-t1")}>{device.Name}</span></div>
        ...
        <div className="w-1/2 border-l border-white/5 bg-bg1/50 flex flex-col overflow-hidden shadow-2xl">
          <div className="p-4 border-b border-white/5 flex items-center justify-between shrink-0 bg-bg1">
            ...
            <div className="flex items-center gap-2">{isFetchingNextPage && <RefreshCcw size={12} className="animate-spin text-t2" />}</div>
          </div>
            ...
            <div className="flex flex-col">
              {auditLogs.map((log, i) => (
                <div key={log.Id || i} className="px-4 py-2.5 border-b border-white/5 last:border-0 hover:bg-white/[0.02] transition-colors flex gap-3 items-start group animate-in slide-in-from-top duration-500">
                  <div className="text-[10px] font-mono text-t-2 w-14 shrink-0 pt-0.5 group-hover:text-t1 transition-colors font-bold">{new Date(log.CreatedAt).toLocaleTimeString('vi-VN', { hour12: false })}</div>
                  <div className="flex flex-col min-w-0">
                    <div className="text-[12px] leading-tight mb-0.5"><span className="font-bold text-psim-accent uppercase tracking-tight">@{log.Username}</span><span className="text-t-2 mx-1.5 opacity-50">•</span><span className="text-t-1 font-medium">{log.ActionType}</span></div>
