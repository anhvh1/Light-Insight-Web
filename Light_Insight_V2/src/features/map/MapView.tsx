        ...
        <div className="relative flex-1 flex overflow-hidden">
          {!activeMap ? (
            ...
            <GeoMapCanvas
              ...
              onMarkerClick={handleAlarmClick}
            ...
            {isExpanded && (
              <div className="p-2 flex flex-col gap-2 max-h-[500px]">
                ...
                <div className="overflow-y-auto custom-scrollbar flex flex-col gap-1 pr-1">
                  {isLoadingTree ? (
                    <RefreshCcw className="animate-spin mx-auto opacity-40" />
                  ) : (
                    filteredMapTree.map((node, i) => (
                      <TreeItem 
                        key={`${node.map.id}-${i}`} 
                        node={node} 
                        level={0} 
            ...
            {latestAlarms.map((alarm) => (
              ...
              >
                ...
                <div className="flex items-center justify-between gap-2 border-t border-white/[0.03] pt-1.5 pl-1">
                  <div className="flex items-center gap-1 text-[9px] text-t3 truncate"><Cctv size={10} /><span className="truncate">{alarm.src || 'Unknown'}</span></div>
