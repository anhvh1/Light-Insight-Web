...
import type { Alarm, AlarmStatus, AlarmType } from '@/types';
...
export function AlarmConsole() {
  ...
  const tabs: (AlarmStatus | 'all')[] = ['all', 'new', 'ack', 'prog'];
  const filterTypes: (AlarmType | 'all')[] = ['all', 'ai', 'lpr', 'acs', 'fire', 'bms'];
          ...
          {filteredAlarms.map((alarm) => (
            <tr 
              ...
              className={cn(
                ...
                selectedAlarm?.id === alarm.id && "bg-[rgba(0,194,255,0.05)] border-l-0",
                alarm.pri === 'critical' && selectedAlarm?.id !== alarm.id && "border-l-[3px] border-psim-red",
                alarm.pri === 'high' && selectedAlarm?.id !== alarm.id && "border-l-[3px] border-psim-orange",
                alarm.pri === 'medium' && selectedAlarm?.id !== alarm.id && "border-l-[3px] border-psim-yellow",
                alarm.pri === 'low' && selectedAlarm?.id !== alarm.id && "border-l-[3px] border-t2",
              )}
  ...
  const renderAlarmDetail = () => (
    <div className="bg-bg0 overflow-y-auto p-4 flex flex-col gap-3 min-w-[360px] border-l border-border-dim">
      {!selectedAlarm ? (
        ...
        <> 
          <div className="flex items-start gap-2">
            <div>
              ...
              <div className="text-[14px] font-semibold mt-0.5 leading-tight">{selectedAlarm.title}</div>
            </div>
            <StatusPill priority={selectedAlarm.pri} className="text-[11px] ml-auto" />
          </div>

          <div className="grid grid-cols-2 gap-1.5">
            <div className="bg-bg2 rounded-md p-2.5 border border-border-dim">
              <div className="text-[9px] font-mono text-t-2 uppercase tracking-wider mb-0.5">Nguồn</div>
              <div className="text-[12px] font-medium">{selectedAlarm.src}</div>
            </div>
            <div className="bg-bg2 rounded-md p-2.5 border border-border-dim">
              ...
              <div className="flex flex-col gap-1.5">
                <div className="flex items-center gap-2">
                  ...
                  <div className="flex-1">
                    <div className="text-[11px] font-medium">LPR mismatch — {selectedAlarm.src}</div>
                    <div className="text-[10px] text-t-2 font-mono">{selectedAlarm.loc}</div>
                  </div>
                  <div className="font-mono text-[10px] text-t-2">{selectedAlarm.time}</div>
                </div>
                {selectedAlarm.corr > 1 && (
                  <div className="flex items-center gap-2">
                    ...
                    <div className="flex-1">
                      <div className="text-[11px] font-medium">Camera phát hiện sự kiện</div>
                      <div className="text-[10px] text-t-2 font-mono">Milestone · AI-VMD</div>
                    </div>
                    <div className="font-mono text-[10px] text-t-2">+2s</div>
                  </div>
                )}
                {selectedAlarm.corr > 2 && (
                  <div className="flex items-center gap-2">
                    ...
                    <div className="flex-1">
                      <div className="text-[11px] font-medium">BMS xác nhận không có booking</div>
                      <div className="text-[10px] text-t-2 font-mono">BMS Portal</div>
                    </div>
                    <div className="font-mono text-[10px] text-t-2">+3s</div>
                  </div>
                )}
              </div>
            </div>
          ...
          <div className="flex gap-1.5">
            ...
            <button 
              ...
              onClick={() => setSelectedAlarm(prev => prev ? { ...prev, status: 'ack' } : null)}
            >
              ✓ Acknowledge
            </button>
          </div>
            ...
            <div className="flex flex-col gap-1.5">
              <div className="flex gap-2.5">
                ...
                <div className="flex-1 pb-1.5">
                  <div className="text-[12px] leading-tight">Alarm tạo tự động — {selectedAlarm.src}</div>
                  <div className="font-mono text-[10px] text-t-2 mt-0.5">{selectedAlarm.time} · Hệ thống</div>
                </div>
              </div>
              <div className="flex gap-2.5">
                <div className="flex flex-col items-center">
    ...
    <div className="flex flex-col h-full bg-bg0">
      <div className="p-3.5 flex items-center gap-2.5 border-b border-border-dim shrink-0">
        ...
        <div className="text-[10px] text-t-2 font-mono ml-auto">
          {filteredAlarms.length} active · {MOCK_ALARMS.filter(a => a.pri === 'critical').length} critical
        </div>
      </div>
