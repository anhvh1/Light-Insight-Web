...
//   const [scalingDeviceId, setScalingDeviceId] = useState<string | null>(null);
//   const [scalingSide, setScalingSide] = useState<'left' | 'right' | 'tip' | null>(null);
...
//           ip: m.IP || '',
//           fovLength: m.FovLength || 100,
//           fovWidth: m.FovWidth || 140
//         };
//       });
...
//       // 4. Device Scaling Logic (FoV)
//       if (scalingDeviceId && selectedMapId && scalingSide) {
//         setPlacedDevices(prev => {
...
//           const rad = (mouseAngle * Math.PI) / 180;
...
//       if (scalingDeviceId) setScalingDeviceId(null);
//     };
...
//     },
//     onError: (error) => {
//       setResponseModal({ isOpen: true, status: -1, message: `Đã xảy ra lỗi: ${error.message}` });
//     }
//   });
...
//     setDraggingDevice({ ...device, vmsId, fovLength: 100, fovWidth: 140 });
...
//   return (
//     <>
//       <div className="flex flex-col gap-6 h-full animate-in fade-in duration-500 w-full overflow-hidden" onClick={() => setSelectedDeviceId(null)}>
//         <div className="flex items-center justify-between shrink-0">
//           <div>
...
//         <div className="flex gap-4 flex-1 overflow-hidden" onClick={(e) => e.stopPropagation()}>
//           {/* Tabbed Sidebar */}
...
//                         Rotation: d.rotation,
//                         IP: d.ip,
//                         FovLength: d.fovLength || 100,
//                         FovWidth: d.fovWidth || 140
//                       }));
//                       saveMarkersMutation.mutate({ MapId: selectedMapId!, Markers: markers });
//                     } catch (error: any) {
//                       setResponseModal({ isOpen: true, status: -1, message: `Lỗi tạo dữ liệu lưu: ${error.message}` });
//                     }
//                   }}
...
//                       const isSelected = selectedDeviceId === device.id || movingDeviceId === device.id || rotatingDeviceId === device.id || scalingDeviceId === device.id;
//                       const length = device.fovLength || 100;
...
//                             {/* Top Action Buttons (Rotate & Delete) */}
//                             <div className={cn(
...
//                                       isSelected ? "opacity-100" : "opacity-0 group-hover/device:opacity-100"
//                                     )}
//                                     onMouseDown={(e) => { e.stopPropagation(); setSelectedDeviceId(device.id); setScalingDeviceId(device.id); setScalingSide('right'); }}
//                                   />
//                                 </div>
//                               </div>
