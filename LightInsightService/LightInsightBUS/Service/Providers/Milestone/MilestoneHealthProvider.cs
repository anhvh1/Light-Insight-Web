...
namespace LightInsightBUS.Service.Providers.Milestone
{
    public class MilestoneHealthProvider
    {
        ...
        public async Task<ConnectorHealth> GetHealthAsync(ConnectorListModel config)
        {
            ...
            try
            {
                // 1. Thử lấy Token (Đo Latency thực tế)
                ...
                if (!string.IsNullOrEmpty(token))
                {
                    ...
                    // 2. Lấy danh sách Camera thực tế
                    var cameras = await GetCamerasInternalAsync(config, token);
                    int total = cameras.Count;
                    int online = cameras.Count(c => c.enabled); // Giả sử dùng field 'enabled' hoặc logic status khác

                    health.Stats = $"{online} / {total}";
                    
                    // 3. Tính toán Progress Bar (Logic yêu cầu)
                    // Bắt đầu từ 100 điểm
                    int score = 100;

                    // Trừ điểm theo Camera (Mỗi camera offline trừ % tương ứng)
                    if (total > 0) {
                        int offlineCount = total - online;
                        score -= (offlineCount * 100 / total);
                    } else {
                        score = 0; // Không có camera nào cũng coi là sức khỏe kém
                    }

                    // Trừ điểm theo Latency
        ...
        public async Task<List<InfrastructureHealth>> GetInfrastructureAsync(ConnectorListModel config)
        {
            ...
            if (!string.IsNullOrEmpty(token))
            {
                ...
                // Lấy danh sách Camera Offline đưa lên đầu
                var cameras = await GetCamerasInternalAsync(config, token);
        ...
        // --- Logic nội bộ (Private) chỉ dành cho Milestone ---
